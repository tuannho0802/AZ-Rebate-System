...
export default function CommissionManager() {
    ...
    const loadDirectChildrenAccounts = async () => {
        if (!ownId) return;
        try {
            // For root (MIB) /users returns the ENTIRE subtree recursively — filter
            // down to DIRECT children only (parentId === ownId), since MIB/IB may
            // only CRUD their own direct children per the golden rule.
            // For non-root (IB) /users already returns [self, ...directChildren].
            const res = await api.get<UserRecord[] | { data: UserRecord[] }>('/users?limit=200');
            const list = Array.isArray(res) ? res : res.data;
            const direct = (list ?? []).filter((u) => u.parentId === ownId);
            setDirectChildren(direct);
        } catch (error) {
